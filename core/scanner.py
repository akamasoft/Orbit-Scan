...
import psutil
from scapy.all import ARP, ICMP, TCP, UDP, Ether, conf, sniff, sr1, srp, srp1
...
def check_root():
    ...
    if hasattr(os, "getuid") and os.getuid() != 0:
        raise PermissionError("Privilèges root requis : lancez le programme avec sudo.")
...
def get_local_subnet(iface_name=None) -> str:
    ...
    if iface_name:
        ...
        if not stats[iface_name].isup:
            raise RuntimeError(f"Interface « {iface_name} » inactive.")
        ...
        raise RuntimeError(f"Aucune adresse IPv4 sur « {iface_name} ».")
    ...
    raise RuntimeError("Aucune interface active trouvée.")
...
def _probe_ttl(ip: str, mac: str | None = None) -> int | None:
    try:
        if mac and ":" in mac:
            reply = srp1(Ether(dst=mac) / ScapyIP(dst=ip, ttl=64) / ICMP(), timeout=1, verbose=False)
        else:
            reply = sr1(ScapyIP(dst=ip, ttl=64) / ICMP(), timeout=1, verbose=False)
        if reply and ICMP in reply and ScapyIP in reply:
...
def _host_has_token(hostname: str, token: str) -> bool:
    normalized = hostname.replace("_", "-").replace(".", "-")
...
def _infer_role(
    ...
    if _host_has_token(h, "ap") or any(
        k in h
        for k in [
            "wifi",
            "wlan",
    ...
    if any(k in v for k in ["raspberry"]) or "raspberry" in h or _host_has_token(h, "rpi"):
        return "raspberry"

    if any(k in v for k in ["vmware", "virtualbox", "proxmox", "parallels"]) or any(
        k in h for k in ["proxmox", "virtualbox", "vmware"]
    ):
        return "vm"
    ...
    if any(k in h for k in ["ipad", "tablet", "kindle"]):
        return "tablet"

    if any(k in h for k in ["macbook", "laptop", "notebook", "thinkpad", "chromebook"]):
        return "laptop"

    if any(k in h for k in ["printer", "laserjet", "deskjet", "officejet", "print"]):
        return "printer"

    if any(k in h for k in ["imac", "macmini", "mac-mini", "mac_mini"]):
        return "pc"

    if any(k in h for k in ["server", "nas", "synology", "qnap", "freenas", "truenas"]) or "-srv" in h:
        return "server"

    if any(k in v for k in ["apple"]) or "apple" in h:
        return "apple"
...
def _enrich_host(host: dict, known_gateway_ip: str | None) -> dict:
    ...
    ttl = _probe_ttl(ip, host.get("mac"))
    host["ttl"] = ttl
